# Check to make sure the delimeters style is compatible
# Use case scenarios demonstrated at the bottom of the page following function definitions
...
fileout = "calculation_out.csv"
with open (fileout, 'a') as csvfile: # Append mode
    csvwriter = csv.writer(csvfile) # Create a csv writer object
    
    for a in range (0,len(yr_title)):
        hdr101 = yr_title[a]
        csvwriter.writerow(hdr101)
        for b in range(0,len(months)):
            tbl_in= get_table_with_title_subtitle_header("testfile1.csv", yr_title[a], months[b], "W1") # "W1" is the column title
            calc = table_rows_mean_std_median(tbl_in)
            calc = convert_table_to_string(calc) # To save memory if needed or comment out
            for row, value in zip (calc, col_to_add):
                row.insert(0, value)
            hdr102= months[b]
            csvwriter.writerow(hdr102)
            hdr103 = workday
            csvwriter.writerow(hdr103)
            for z in range (0, len(calc)):
                tble_row = calc[z]
                csvwriter.writerow(calc[z])
